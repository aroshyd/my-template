...
import uz.personal.mapper.BaseObjectMapper;
...
public abstract class GenericDao<T extends Auditable, C extends GenericCriteria> {
    ...
    public T find(Long id) {
        try {
            return entityManager.createQuery("Select t from " + persistentClass.getSimpleName() + " t where t.state <> 2 and t.id = " + id, persistentClass).getSingleResult();
